...
fisher_LC <- rast('Fisher_land_cover.tif')
plot(fisher_LC)
...
popden <- rast("Fisher_population_density.tif")
plot(popden)
points(telem_data[,c("x_","y_")])
...
used_elev <- extract(elevation, used_vect)
head(used_elev)
...
# Lets combine all our used data into a single data frame. We will come back
# to that later.
...
# We now have data for one of two parts of a resource selection function. The 
# second part is the available data. Based on what we have discussed so far
# what are some ways in which we might measure habitat availability? 
...
# Lupe's minimum convext polygon (MCP) home range. How many random points do we
# need? It depends but the standard advice is a lot. Lets select 10 random
# points for every used point.

system.time(avail_vect <- spatSample(fisher_MCP, 
                                     size = nrow(telem_data)*10,
                                     method="random"))
avail_vect
nrow(avail_vect)
...
points(telem_data[,c("x_","y_")], cex = 0.75, pch = 21, bg = "grey")
...
# used and available locations. Notice any patterns?

boxplot(Elevation ~ y, all_data)
boxplot(PopDen ~ y, all_data)

# To compare usage vs. availability with our categorical land covers we will 
# compare the proportion of points within each category.
...
# Now we can fit our resource selection function. We will do this using a
# binomial GLM but again we ARE NOT MODELING BINOMIAL DATA! We will come back
# to the why behind this but for now I want to illustrate the model fitting
# process and look at the coefficients. 

RSF_global <- glm(y ~ Elevation + PopDen + Wetland + Grassland, 
                  data = all_data, 
                  weight = w,
