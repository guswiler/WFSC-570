...
# Continuing Lab 3: Introduction generalized linear models (GLMs)
...
# plotting our estimated intercept
abline(v = 0)
points(x = 0,
       y = est_intercept,
       pch = 21,
...
# how much does y change across one unit on the x-axis
# so this expression is calculating the rate of change in y over one unit of x, so the slope
(est_slope*1 + est_intercept) - (est_slope*0.00 + est_intercept)



# How do our estimates of intercept and slope compare to our true values? We know these because simulated data
b0
est_intercept
...
# Why do we observe differences between the true values and our estimates?
  # variability in data, particularly in smaller sample size. As you increase n, our estimates become closer to the actual values

# main takeaways? from here are estimating slopes and intercepts from a normal distribution, what goes into them and effects the precision of our estimates
...
# In a study that yields binary response data, for example presence/absence data from a habitat selection study or whether or not an individual exhibited symptoms of a disease, the error distribution that naturally accommodates the discrete and 0,1 bounding of the data is the binomial distribution.

# The binomial distribution has two parameters: p, the per trial probability of success, and n, the trial size. 
  # You can think of n as the number of times you flip a single coin 
  # and p as the probability of getting heads on any given flip (p = 0.5).

# A special case of the binomial distribution is when each coin is only flipped once
  # i.e., the trial size is n = 1, and therefore produces binary data.
  # The binomial distribution where n = 1 is called the Bernoulli distribution. 
    # The expected value (mean) of the binomial distribution is n*p, so it follows that the mean of the Bernoulli distribution is simply p.

# For most applications of the binomial distribution in wildlife habitat analysis one will use the Bernoulli distribution.



# Lets illustrate the Bernoulli distribution using the rbinom() function.

# rbinom() is the function to simulate random values according to a binomial distribution
...
# size is the trial size (number of coin flips), for bernoulli, always 1
# and p is the per trial probability. 
# Remember that we can think of p as the probability of getting heads (getting a 1) for any given coin flip.
...
y <- rbinom(n = 1000, # number of random numbers to simulate (how many coins)
            size = 1, # trial size (how meany times we flipped each coin)
...
mean(y) # expected value is pretty close to our true value
# smaller your n the more imprecise parameter estimates will be
...
# link "logit" allows us to treat our discrete binomial data as continuous

# Which is the same as, because logit is the assumed link for binomial family
mod <- glm(y ~ 1, family = "binomial")

# Lets see what our expected value is
summary(mod)

# What do we see here? INTERCEPT

# We only have an intercept value (no slope) because we have an intercept-only model.
# It is on the logit scale, not the probability scale, so we need to do some back-transformation to get our intercept onto the probability scale.

# We do this using the logit transformation.
# We can do this "manually" by dividing the exponent of our intercept by one plus the exponent of the intercept.

b0 <- coef(mod)

exp(b0)/(1 + exp(b0)) # this is our intercept on the probability scale
...
# We will use a large sample size to cut down on the variability in our estimated parameters.
n <- 500   # e.g. number of sites we do point counts
...
lin_pred <- b0 + b1 * X   # calculating our expected values (this is our deterministic function)

plot(x = X,
     y = lin_pred,
     xlab = "X",
     ylab = "Y")
# look familiar? expected y for any given value of x



# We can use the plogis() function to see our linear predictor values on the probability scale
link <- plogis(lin_pred) # essentially, plogis(b0 + b1 * X) running plogis() on our deterministic function
summary(link)
# can see we're now on a continuous scale between 0 and 1
# back on the probability scale

# What happens if X = 0 in this example?
plogis(b0 + b1 * 0)
# How does this compare to the smallest value we see in "link?"
  # very close to our smallest value in the link
  # odds of finding grasshopper sparrows at a plot with 0% forest cover, we expect to observe grasshopper sparrows near 1% of the time
plogis(b0 + b1 * 100)
# odds of finding grasshopper sparrows at a plot with 100% forest cover, we expect to observe grasshopper sparrows near 100% of the time


# Now lets simulate our response variable using rbinom.
# We are using a trial size of one because we are simulating Bernoulli trials (i.e., coin flips), such as we would have if we were doing presence/absence surveys at a site. 

y <- rbinom(n,    # Number of samples
            1,    # Trial size (one because we have Bernoulli
                  # trials or coin flips)
            link) # Corresponding probabilities, per trial for each sample: plogis(b0 + b1 * X). the stochastic component of our model!

# You can see that we have simulated a vector of binary data representing the outcome of each Bernoulli trial (i.e., of each coin flip).
table(y)

# Lets plot these binary data against our covariate X (grass cover)
plot(y ~ X, 
     ylim=c(0,1), 
     pch=16, 
     col=adjustcolor(4,alpha.f = 0.2), # making semi-transparent points
     cex=1.5, 
     main="Binary data")


# How can we model these binary data as a continuous linear function of our covariate, X?   # By using the link function.
...
# Remember that these are on the logit scale, not the probability scale.
# But lets stay on the logit scale to show that, as long as
...
# What is our slope? (moving one unit on the x-axis)
...
# Gaussian linear regression and GLMs

# but in the real world we typically want to see how per-trial probability varies as a function of our covariates.
# To do this, we need to be able to calculate expected values on the probability scale. Fortunately, this is relatively straight-forward to do.

# Here, we can illustrate these calculations using the plogis() function.
plogis(est_b0 + est_b1*0)
plogis(est_b0 + est_b1*1)   # 1 unit change in grass cover, 4% chance of getting bird
plogis(est_b0 + est_b1*10)  # 10 unit change in grass cover, 4% chance of bird
plogis(est_b0 + est_b1*50)  # 50 unit change in grass cover, 50% chance of getting bird
plogis(est_b0 + est_b1*90)
plogis(est_b0 + est_b1*100)
# we can see our data is non-linear

# Notice some major non-linearities in our expected probabilities?
# This is because the plogis() function ensures that our probabilities stay bounded between 0 and 1, which is appropriate given that our actual response variable is made up of 0's and 1's.




# Lets show how we can use the predict() function to calculate these expected values for us. predict() is a pretty useful function and if we look at the help we will see that there are several arguments, only three of which need to concern us for now.
?predict.glm

# We need to supply a glm object,
# a data frame containing the value(s) of our covariates at which we would like to calculate the expected values,
# and type=, which we use to specify whether we want these expected values returned on the link (i.e., logit) scale or the response (i.e., probability) scale. 
...
# also predicting on the response scale to get our probabilities
df$response <- predict(bin.glm, newdata=df, type="response")
head(df)
tail(df)


# Now lets re-plot our simulated data and add our expected values as lines.
plot(y ~ X, 
     ylim=c(0,1), 
     pch=16, 
     col=adjustcolor(4,alpha.f = 0.3),
     cex=1.5, 
     main="Grasshopper Sparrow Occupancy",
     xlab = "Percent Grass Cover (%)",
     ylab = "Probability of Occupancy")

lines(df$X, df$lin.pred, col="grey", lwd=2) # on the logit scale
lines(df$X, df$response, col="red", lwd=2)  # on the probability scale

# back-transforming from the logit to the probability scale gives us our non-linear curve

# depending on our curve, we're able to tell stakeholders that there is a positive relationship between bird occupancy and % grass cover. There is a kind of threshold point, where after adding more grass cover, there is not much influence on occupancy of grasshopper sparrows.
# at sites with 20% grass cover, we would expect only 5% of the sampled sites to have grasshopper sparrows present